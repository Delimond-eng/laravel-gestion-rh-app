...
class HolidaysService {
    ...
    */
    ...
    /**
     * Calcul les jours ouvrables en excluant les jours fériés et les jours de weekend
     * @param $startDate
     * @param $endDate
     * @return integer
     */
    public function calculateWorkingDays($startDate, $endDate): int
    {
        ...
        while ($start->lte($end)) {
            if (!$this->isWeekend($start) && !$this->isHoliday($start)) {
                $workingDays++;
            }
            $start->addDay();
        }
    ...
    /**
     * Renvoyer la date de la fin du congé en fonction de la date du debut et le nbre de jours attribués
     * @param $startDate
     * @param $totalLeaveDays
     * @return Carbon
     */
    public function calculateEndDate($startDate, $totalLeaveDays): Carbon
    {
        $start = Carbon::parse($startDate)->startOfDay();
        $daysCounted = 0;

        while ($daysCounted < $totalLeaveDays) {
            if (!$this->isWeekend($start) && !$this->isHoliday($start)) {
                $daysCounted++;
            }
            $start->addDay();
        }
        $endDate = $start->subDay();
        return $endDate;
    }
